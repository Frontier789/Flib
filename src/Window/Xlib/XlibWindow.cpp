...
namespace fw
{
	namespace priv
	{
		fm::Result getFBConfig(Display *disp,void *fbconfptr,bool window)
		{
			int glx_major = 0;
			int glx_minor = 0;
			...
			{
				return fm::Result("XError",fm::Result::OPFailed,"FBConfigNotSupported","glXQueryVersion",__FILE__,__LINE__,"glx "+fm::toString(glx_major)+","+fm::toString(glx_major));
			}
			...
			if (!fbConfigs)
			{
				...
				if (!fbConfigs)
					return fm::Result("XError",fm::Result::OPFailed,"UnknownXError","glXChooseFBConfig",__FILE__,__LINE__);
			}
	...
	namespace Xlib 
	{
			...
			if (xev.type ==  SelectionNotify)
			{
			    ...
			    if (m_supportUriList)
			    {
			    	...
			    	for (fm::Size i=0;i<length;i++)
			    	{
			    		...
			    		if (msg[i] == '\n')
			    		{
							std::string fileStr2 = "file://localhost";
							std::string fileStr1 = "file://";
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::open(int x,int y,unsigned int w,unsigned int h,const fm::String &title,unsigned int style,Xlib::Window *parent,::Window container)
		{
			if (!checkDisplay())
				return fm::Result("XError",fm::Result::OPFailed,"NoDisplay","checkDisplay",__FILE__,__LINE__);
			...
			GLXFBConfig fbConf;
			fm::Result res = priv::getFBConfig(m_disp,&fbConf,true);
			...
			if (m_win != (::Window)NULL)
			{
				...
				char emptyCur[] = {0, 0, 0, 0, 0, 0, 0, 0};
				Pixmap pmap   = XCreateBitmapFromData(m_disp,m_win,emptyCur, 8, 8);
				...
				if (!xim)
				    return fm::Result("XError",fm::Result::OPFailed,"NoInputMethod","XOpenIM",__FILE__,__LINE__);
				

				m_xic = XCreateIC(xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow, m_win, NULL);
				if (!m_xic)
				    return fm::Result("XError",fm::Result::OPFailed,"UnknownXError","XCreateIC",__FILE__,__LINE__);
			...
			return fm::Result("XError",fm::Result::OPFailed,"UnknownXError","XCreateWindow",__FILE__,__LINE__);
		}

		////////////////////////////////////////////////////////////
		fm::Result Window::open(unsigned int w,unsigned int h,const fm::String &title,unsigned int style,Xlib::Window *parent,::Window container)
		{
			if (!checkDisplay())
				return fm::Result("XError",fm::Result::OPFailed,"NoDisplay","checkDisplay",__FILE__,__LINE__);
