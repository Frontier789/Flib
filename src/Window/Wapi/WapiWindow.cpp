...
{
	std::cout << "destroying " << h << std::endl;
	return DestroyWindow(h);
}

HWND WINAPI createAndPromptWex(DWORD exStyles,LPCTSTR lpClassName,LPCTSTR lpWindowName,DWORD dwStyle,int x,int y,int nWidth,int nHeight,HWND hWndParent,HMENU hMenu,HINSTANCE hInstance,LPVOID lpParam)
{
	std::cout << "excreating " << lpWindowName << std::endl;
	HWND ret = CreateWindowEx(exStyles,lpClassName,lpWindowName,dwStyle,x,y,nWidth,nHeight,hWndParent,hMenu,hInstance,lpParam);
	std::cout << "created with id " << ret << std::endl;
	return ret;
}


#define DestroyWindow destroyAndPromptW
#define CreateWindowExA createAndPromptWex
*/
...
namespace fw
{
	...
	/////////////////////////////////////////////////////////////
	fm::Result adjustWindowSize(unsigned int &w,unsigned int &h,DWORD style,DWORD exStyle)
	{
		RECT Rct;
		Rct.top    = 100;
		Rct.left   = 100;
		...
		if (!AdjustWindowRectEx(&Rct,style,FALSE,exStyle))
			return fw::WapiGetLastError("AdjustWindowRectEx");
	...
	namespace Wapi
	{
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::open(int x,int y,unsigned int w,unsigned int h,const fm::String &title,unsigned int style,Wapi::Window *parent,HWND container)
		{
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::open(unsigned int w,unsigned int h,const fm::String &title,unsigned int style,Wapi::Window *parent,HWND container)
		{
			int x,y;
		...
		/////////////////////////////////////////////////////////////
		fm::Result Window::setActive()
		{
			if (m_hwnd)
			{
				// bring the window to front
				if (!SetActiveWindow(m_hwnd))
					return fw::WapiGetLastError("SetActiveWindow");
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::setRect(int x,int y,unsigned int w,unsigned int h)
		{
			if (m_hwnd)
			{
				fm::Result res = adjustWindowSize(w,h,GetWindowLong(m_hwnd,GWL_STYLE),GetWindowLong(m_hwnd,GWL_EXSTYLE));
				...
				if (!SetWindowPos(m_hwnd, // target HWND
						...
						return fw::WapiGetLastError("SetWindowPos");
			}
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::getRect(int &x,int &y,unsigned int &w,unsigned int &h)
		{
			if (m_hwnd)
			{
				RECT client_rect;
				...
				if (!ClientToScreen(m_hwnd,(POINT*)&client_rect.left) || !ClientToScreen(m_hwnd,(POINT*)&client_rect.right))
					return fw::WapiGetLastError("ClientToScreen");
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::setPosition(int x,int y)
		{
			if (m_hwnd)
				if (!SetWindowPos(m_hwnd, // target HWND
								  NULL,   // Z-order specifier
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::getPosition(int &x,int &y) const
		{
			if (m_hwnd)
			{
				RECT client_rect;
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::setSize(unsigned int w,unsigned int h)
		{
			if (m_hwnd)
			{
				if (!adjustWindowSize(w,h,GetWindowLong(m_hwnd,GWL_STYLE),GetWindowLong(m_hwnd,GWL_EXSTYLE)))
					return fw::WapiGetLastError("adjustWindowSize");
		...
		////////////////////////////////////////////////////////////
		fm::Result Window::getSize(unsigned int &w,unsigned int &h) const
		{
			if (m_hwnd)
			{
				RECT client_rect;
		...
		fm::Result Window::setTitle(const fm::String &title)
		{
			if (m_hwnd)
				if (!SetWindowTextW(m_hwnd,&title.wstr()[0]))
					return fw::WapiGetLastError("SetWindowTextW");

			return fm::Result();
		}

		////////////////////////////////////////////////////////////
		fm::Result Window::getTitle(fm::String &title) const
		{
			if (m_hwnd)
			{
				...
				if (!written)
				{
					...
					delete ret;
					return fw::WapiGetLastError("GetWindowTextW");
				}
				title = fm::String(ret);
		...
		{
			if (m_hwnd)
				if (!ShowWindow(m_hwnd,visible ? SW_SHOW : SW_HIDE))
					return fw::WapiGetLastError("ShowWindow");
		...
		void Window::setIcon(const fg::Image &icon)
		{
			if (m_hwnd)
			{
				...
				// convert to HICON
				m_icon = CreateIcon(NULL, width, height, 1, sizeof(fg::Color)*8, NULL, (unsigned char*)&iconColors[0]);
