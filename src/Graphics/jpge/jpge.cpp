...
unsigned char *jpeg_write_mem(const unsigned char *dataPtr,int width,int height,int channels,int *bytesWritten)
{
	unsigned char *buf = new unsigned char[width*height*4];
	
	int bufSize = width*height*4;
	
	if (!compress_image_to_jpeg_file_in_memory(buf,bufSize,width,height,channels,dataPtr))
	{
		delete[] buf;
	...
	*bytesWritten = bufSize;
	...
	{
		unsigned char *bufCpy = new unsigned char[bufSize];
